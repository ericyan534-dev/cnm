...
# Regex patterns for IDS sanitization
...
def normalize_ids_mapping(raw) -> dict:
    ...
    Handles values:
      - string IDS: "⿰亻尔"
      - list of IDS strings: ["⿰亻尔", "⿰亻尓"]
      - dict with ids field: {"ids": "⿰亻尔", ...}
      - list tokens: ["⿰", "亻", "尔"]
    """
    def key_to_char(k) -> Optional[str]:
...
def fail_fast_ids_mapping(ids_data: dict) -> None:
    """
...
def fail_fast_parser(parser_instance: IDSParser) -> None:
    """
    Validate parser produces non-leaf trees for common characters.
    Raises RuntimeError if validation fails.
    """
    tests = ["你", "好", "我", "中", "国"]
    bad = []
    for ch in tests:
        try:
            t = parser_instance.parse(ch)
            # Check if tree is a leaf (no decomposition)
            if t is None or t.depth == 0 or t.operator is None:
                bad.append((ch, t))
